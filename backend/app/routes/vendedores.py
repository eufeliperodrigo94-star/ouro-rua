...
async def list_vendedores(user=Depends(get_current_user)):
    q = supabase.table("vendedores").select(FIELDS).order("name")
...
async def get_vendedor(vendedor_id: int, user=Depends(get_current_user)):
    res = supabase.table("vendedores").select(FIELDS).eq("id", vendedor_id).single().execute()
    if not res.data:
...
async def create_vendedor(body: dict, user=Depends(get_current_user)):
    ...
    res = supabase.table("vendedores").insert(row).execute()
...
async def update_vendedor(vendedor_id: int, body: dict, user=Depends(get_current_user)):
    ...
    updates.pop("id", None)
    res = supabase.table("vendedores").update(updates).eq("id", vendedor_id).execute()
    return res.data[0] if res.data else {}

@router.delete("/{vendedor_id}")
async def delete_vendedor(vendedor_id: int, user=Depends(get_current_user)):
    supabase.table("vendedores").update({"is_active": False}).eq("id", vendedor_id).execute()
    return {"ok": True}
...
async def change_password(vendedor_id: int, body: dict, user=Depends(get_current_user)):
    ...
    res = supabase.table("vendedores").select("password_hash").eq("id", vendedor_id).limit(1).execute()
    if not res.data:
        raise HTTPException(status_code=404, detail="Usuário não encontrado")
    if body.get("senha_atual"):
        if not pwd_context.verify(body["senha_atual"], res.data[0]["password_hash"]):
            raise HTTPException(status_code=401, detail="Senha atual incorreta")
    supabase.table("vendedores").update({"password_hash": pwd_context.hash(nova_senha)}).eq("id", vendedor_id).execute()
    return {"ok": True}
