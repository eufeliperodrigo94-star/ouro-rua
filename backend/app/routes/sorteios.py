...
async def list_sorteios(
    ...
    q = supabase.table("sorteios").select("*, extracoes(name)") \
...
async def get_sorteio(sorteio_id: int, user=Depends(get_current_user)):
    res = supabase.table("sorteios").select("*, extracoes(name)") \
...
async def create_sorteio(body: dict, user=Depends(get_current_user)):
    res = supabase.table("sorteios").insert(body).execute()
    if not res.data:
...
async def update_sorteio(sorteio_id: int, body: dict, user=Depends(get_current_user)):
    res = supabase.table("sorteios").update(body).eq("id", sorteio_id).execute()
    if not res.data:
...
async def processar_resultado(sorteio_id: int, body: dict, user=Depends(get_current_user)):
    ...
    supabase.table("sorteios").update({"resultado": resultado, "status": "closed"}) \
        .eq("id", sorteio_id).execute()
    supabase.table("apostas").update({"status": "pending", "prize_amount": 0}) \
        .eq("draw_id", sorteio_id).in_("status", ["pending", "won", "lost"]).execute()
    res = supabase.rpc("processar_resultado", {"p_sorteio_id": sorteio_id}).execute()
